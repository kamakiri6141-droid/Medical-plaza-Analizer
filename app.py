...
if df is not None:
    try:
        ...
        # 生データからの列自動検出（金額・日付列など）は、実データの列構成によっては集計精度が不安定なため、
        # 生データ由来の売上集計（KPI行・項目別売上・月次推移）は常に非表示にする。
        # 売上サマリーは、正確な年次サマリー表（ルートC）が入力された場合にのみ表示する。
        _show_auto_sales_summary = False
        ...
        with col1:
            ...
            if not _show_auto_sales_summary:
                st.caption("生データからの売上自動集計は精度の都合により非表示にしています。売上のサマリーを確認するには「年次サマリー表を追加する」欄からアップロードしてください。")
